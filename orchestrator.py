...
def main():
    ...
    print(f"{COLOR_SYS} [1/3] IPEX GPU Runner başlatılıyor (Port {IPEX_RUNNER_PORT} / 32K Context)...", flush=True)
    ...
    print(f"{COLOR_SYS} [2/3] Embedding Ollama Servisi başlatılıyor (Port 11435)...", flush=True)
    env_embed = env.copy()
    ...
    # 3. Flask App (Port 5000)
    print(f"{COLOR_SYS} [3/3] GK STUDIO V3 (app.py) başlatılıyor...", flush=True)
